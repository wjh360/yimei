...
async function generateExcel(rows, table) {
  ...
  // 定义表头字段
  const headers = ["导师", "店面名称", "时间", "操作顾客名单", "操作项目", "业绩", "回款情况", "分成", "咨询", "操作", "商务"];
  
  // 深拷贝数据以避免修改原始数据
  const dataRows = JSON.parse(JSON.stringify(rows));
  
  // 生成文件名（使用月份信息）
  const fileName = (dataRows[0].date.split(".")[0]) + "月份账目报表【工资情况】";
  ...
  const mergeColumns = (sheet, headers) => {
    // 数字列不合并
    const numericColumns = ['业绩', "操作项目", '回款情况', '分成', "商务"];
  ...
  totalRow.getCell(6).value = table.allYeji;
  totalRow.getCell(7).value = table.allechoMoney;
  totalRow.height = 30;
  ...
  if (headers.length > 7) {
    worksheet.mergeCells(
      totalRow.number, 8,
      totalRow.number, headers.length
    );
    
    // 设置合并后单元格的值和样式
    const mergedCell = totalRow.getCell(8);
    mergedCell.value = table.remark || "";
